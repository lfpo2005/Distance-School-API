...
public class UserCourseController {
    ...
    public ResponseEntity<Page<CourseDto>> getAllCoursesByUsers(@PageableDefault(page = 0, size = 10, sort = "courseId", direction = Sort.Direction.ASC) Pageable pageable,
    ...
    public ResponseEntity<Object> saveSubscriptionUserInCourse(@PathVariable(value = "userId") UUID userId,
        ...
        if (userCourseService.existsByUserAndCourseId(userModelOptional.get(), userCourseDto.getCourseId())){
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Error: user already exists!");
        }
